...
FROM product.products p
    ...
    try:
        ...
        # 簡單錯誤處理，實作可改為記錄或重新拋出
        logger.error(f"{fetch_products.__name__} DB error: {e}")
    finally:
...
WHERE p.id = %s
    ...
    try:
        ...
        logger.error(f"{fetch_product_by_id().__name__} DB error: {e}")
    finally:
        if conn and conn.is_connected():
            conn.close()
    return None
...
async def fetch_products_async() -> List[ProductRecord]:
    """"
    非同步撈取多筆產品資料。
    """
...
FROM product.products p
    ...
    try:
        ...
        logger.error(f"{fetch_products_async().__name__} DB error: {e}")
    finally:
        if conn:
            conn.close()
    return results
...
async def fetch_product_by_id_async(product_id: int) -> ProductRecord | None:
    """
    根據 product_id 非同步撈出一筆 ProductRecord，找不到或發生錯誤則回傳 None。
    """
    sql = """
SELECT p.id AS product_id, p.sale_code, pan.alias_name, pp.pic
FROM product.products p
...
WHERE p.id = %s
    ...
    try:
        ...
        logger.error(f"{fetch_product_by_id_async.__name__} DB error: {e}")
    finally:
        if conn:
            conn.close()
    return None
